...
// fid -> function id
// eid -> extension id
// All registers except a0 & a1 must be preserved across an SBI call by the callee (OpenSBI).
struct sbiret sbi_call(long arg0, long arg1, long arg2, long arg3, long arg4, long arg5, long fid, long eid) {
    ...
    // function call to the more privileged CPU mode
...
long sbi_console_putchar(int ch) {
    struct sbiret ret = sbi_call(ch, 0, 0, 0, 0, 0, 0, 0x01);
    return ret.error;
}
...
void switch_context(uint32_t* prev_sp, uint32_t* next_sp) {
    // Saves and restores callee-saved registers only
    __asm__ __volatile__(
        "addi sp, sp, -13 * 4\n"
        "sw ra,  0  * 4(sp)\n"   
        "sw s0,  1  * 4(sp)\n"
...
void yield(void) {
    ...
    // Set sscratch as current process's kernel stack
    __asm__ __volatile__(
...
// Copies all file contents into `disk` buffer
void fs_flush(void) {
    ...
    for (int file_i = 0; file_i < FILES_MAX; file_i++) {
        ...
        for (unsigned i = 0; i < sizeof(struct tar_header); i++)
            checksum += (unsigned char) disk[off + i]; // header[i]
...
void kernel_main(void) {
    ...
    // read_write_disk(buf, 0, false); // R
    // printf("first sector: %s\n", buf);

    // strcpy(buf, "hello from kernel!!!\n");
    // read_write_disk(buf, 0, true); // W
